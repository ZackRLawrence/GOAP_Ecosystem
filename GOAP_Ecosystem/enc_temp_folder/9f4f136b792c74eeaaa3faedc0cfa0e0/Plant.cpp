...
{
	APlant* child = this->GetWorld()->SpawnActor<APlant>(PlantToSpawn, location, FRotator(0));
	if (child)
	{
		float childSize = m_relativeSize + FMath::FRandRange(-.05f, .05f);
		float childNutrition = m_nutritionMultiplier + FMath::FRandRange(-.05f, .05f);
